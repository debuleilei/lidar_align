...
std::shared_ptr< adu::common::sensor::PointCloud>& point_cloud_fusion){ 
  ...
  {  
      //  Eigen::Isometry3d T = Eigen::Isometry3d::Identity();
       ...
       double angle_rad=INSPVA->azimuth()*(PI/180);
        ...
        T(1,3)=utm_ins.y;
        T(2,3)=0; 
        // Eigen::Quaterniond Q1(rotate_matrix);
  ...
  if(m_back.angle.size()==0)
  {
    ...
    *data_befor_b=*lidar; 
    ...
    // pcl::io::savePLYFile ("up.ply", *lidar);
    a_back.filter_floor(lidar,Normal_floor);
  ...
  if(abs(INSPVA->azimuth()-m_back.angle.back())>5&& !m_back.init_calib )
  {
    *data_b=*lidar;    
    Eigen::Affine3d tt;   
    ...
    //  cout<<"点云turn:"<<eulerAngle<<endl;
      ...
      double deta_angle;
      deta_angle=INSPVA->azimuth()-m_back.angle.back();
    ...
    {   
        ...
        t=T.inverse()*m_back.T_list[i-i];
        Eigen::Isometry3d t_inv=t.inverse();

        Eigen::Vector2d lidar_len,ins_len;
        ins_len<<t_inv(0,3),t_inv(1,3);
        lidar_len<<tt(0,3),tt(1,3);

        double len=ins_len.norm()-lidar_len.norm();
        
        if(INSPVA->azimuth()-m_back.angle[i-1]<0 && m_back.zheng &&abs(len)<0.8)
        {

          //CHECK THE result
          pcl::PointCloud<pcl::PointXYZ>::Ptr data_trans(new pcl::PointCloud<pcl::PointXYZ>);    
          pcl::transformPointCloud(*data_b,*data_trans, tt);
          *data_trans+=*data_befor_b;
          pcl::io::savePLYFile ("see.ply", *data_trans);
      ...
      //   Eigen::Isometry3d t_inv=t.inverse();        
       ...
       cout<<"lidar:"<<tt.matrix()<<endl;
        ...
        if(INSPVA->azimuth()-m_back.angle[i-1]>0 && m_back.fu && abs(len)<0.1)
        {
          pcl::PointCloud<pcl::PointXYZ>::Ptr data_trans(new pcl::PointCloud<pcl::PointXYZ>);    
          pcl::transformPointCloud(*data_b,*data_trans, tt);
          *data_trans+=*data_befor_b;
          pcl::io::savePLYFile ("seefu.ply", *data_trans);
       ...
       if(!m_back.zheng && !m_back.fu && m_back.ss)
      {
        ...
        cout<<" init back result finished*******!!!!!!"<<endl;
        //return true;
        m_back.ss=false;
      } 
  ...
  if(m_back.init_calib && m_back.start_save)
  {
    ...
    if(m_back.init_calib)
    {
      ...
      // cout<<"开始发布"<<point_cloud_first->measurement_time()<<endl;
      // cout<<"点云数量："<<lidar_result->points.size()<<endl;
      // cout<<"cybertron点云数量："<<point_cloud_fusion->point_size()<<endl;
      return 1;
    }        
