...
std::shared_ptr< adu::common::sensor::PointCloud>& point_cloud_fusion){ 
  ...
  if(m_back.T_list.size()==0)
  {
    *data_befor_b=*lidar; 
    ...
    m_back.imu_odo.push_back(imu_ini);

    Eigen::Matrix4d lidar_ini = Eigen::Matrix4d::Identity();
    m_back.lidar_odo.push_back(lidar_ini);
  ...
  if( m_back.T_list.size()>0 && !m_back.init_calib )
  {
    
      m_back.T_list.push_back(T);
      Eigen::Isometry3d t = Eigen::Isometry3d::Identity();
      int i=m_back.T_list.size();     
      t=T.inverse()*m_back.T_list[i-2];

      Eigen::Isometry3d imu_odometry;     
      imu_odometry=m_back.imu_odo.back()*t.inverse();
      m_back.imu_odo.push_back(imu_odometry);
      
      Eigen::Affine3d t_lidar;
      *data_b=*lidar;
      es_back.estimate(*data_befor_b,*data_b,t_lidar);
      *data_befor_b=*lidar;

      Eigen::Matrix4d lidar_odometry,t_lidar_matrix;
      t_lidar_matrix=t_lidar.matrix();     
      lidar_odometry=m_back.lidar_odo.back()*t_lidar_matrix;
      m_back.lidar_odo.push_back(lidar_odometry);
    
      if(INSPVA->azimuth()-m_back.angle.back()>10 && m_back.iter_z<4 )
      {
        m_back.angle.push_back(INSPVA->azimuth());

        Eigen::Isometry3d t_imu = Eigen::Isometry3d::Identity();
          
        t_imu=m_back.imu_odo[m_back.last].inverse()*imu_odometry;
    
        if(abs(t_imu(1,3))<1)
        {
          cout<<"imu:"<<t_imu.matrix()<<endl;
          Eigen::Matrix2d R;
          R<<t_imu(0,0),t_imu(0,1),t_imu(1,0),t_imu(1,1);
          ...
          pc_transform=m_back.lidar_odo[m_back.last].inverse()*lidar_odometry;
          ...
          m_back.lidar_vec.push_back(tlidar);

          m_back.last=i;
          cout<<"back zlast:"<<m_back.last<<endl;
          ++m_back.iter_z;
        }
        //else?????
      }
      ...
      if(INSPVA->azimuth()-m_back.angle.back()<-10 && m_back.iter_f<4 )
      {
        m_back.angle.push_back(INSPVA->azimuth());
        ...
        if(abs(t_imu(1,3))<1)
        {
          ...
          m_back.last=i;
          cout<<"back flast:"<<m_back.last<<endl;
          ++m_back.iter_f;
        }
        else
        {
      ...
      if(m_back.iter_z==4 && m_back.iter_f==4)
        {
         
          m_back.result_init=m_back.sent_x_y_angle(m_back.R_vec,m_back.lidar_vec,m_back.tins_vec,m_back.lidar_height);
          cout<<"result_init="<<m_back.result_init<<endl;
    ...
    if(m_back.init_calib)
    {
        if(m_back.init_add)
        {
          ...
          *tmp_b=*lidar;
          ...
          cout<<"计算真值为："<<m_back.result_init.matrix() << endl;         
        }
        else
        {
          m_back.angle.push_back(INSPVA->azimuth());
...
std::shared_ptr< adu::common::sensor::PointCloud>& point_cloud_fusion){ 
  ...
  if(abs(INSPVA->azimuth()-m_left.angle.back())>10&& !m_left.init_calib )
  {
    ...
    *data_l=*lidar;    
    Eigen::Affine3d tt;   
    es_left.estimate(*data_l,*data_befor_l,tt);
    ...
    *data_befor_l=*lidar;
    ...
    {       
        ...
        m_left.lidar_vec.push_back(tlidar);
        m_left.zheng_iter++;
       ...
       if(!m_left.zheng && !m_left.fu && m_left.ss)
      {
        m_left.zheng=true;
        m_left.fu=true;
        m_left.result_init=m_left.sent_x_y_angle(m_left.R_vec,m_left.lidar_vec,m_left.tins_vec,m_left.lidar_height);
...
std::shared_ptr< adu::common::sensor::PointCloud>& point_cloud_fusion){ 
  ...
  if(abs(INSPVA->azimuth()-m_right.angle.back())>10&& !m_right.init_calib )
  {
    ...
    *data_r=*lidar;    
    ...
    es_right.estimate(*data_r,*data_befor_r,tt);
    ...
    *data_befor_r=*lidar;
    ...
    {     
        ...
        m_right.lidar_vec.push_back(tlidar);
        m_right.zheng_iter++;
       ...
       if(!m_right.zheng && !m_right.fu && m_right.ss)
      {
        m_right.zheng=true;
        m_right.fu=true;
        m_right.result_init=m_right.sent_x_y_angle(m_right.R_vec,m_right.lidar_vec,m_right.tins_vec,m_right.lidar_height);
