...
bool save_odom(string str,vector<Eigen::Isometry3d>&T,vector<double> time_stamp)
{
  ...
  for (int i=0;i<T.size();++i) {
    ...
    Eigen::Matrix4d odom=T[i].matrix();
    const int64_t timestamp_nanoseconds = time_stamp[i];    
    ...
    csv_file << timestamp_nanoseconds << kDelimiter << i
                  ...
                  const Eigen::Affine3d& lidar_extrinsic_,string path)
{
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr back(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    Eigen::Affine3d l_b;    
    pcTransEstimator es;
    es.estimate(*back,*lidar,l_b);
    cout<<"l_b:"<<l_b.matrix()<<endl;
  //save the result
  {
    Eigen::Matrix4d result;
    result=l_b.matrix()*lidar_extrinsic_.matrix();
    write_result write;
   ...
   pcl::io::savePLYFile ("back.ply", *back);
    ...
    pcl::transformPointCloud( *lidar, *left_t, l_b.matrix());
   ...
   if(m.T_list.size()>0 && !m.init_calib)
  {
      ...
      *data_befor_back=*lidar;
      ...
      //*********************************pcd csv*******************************************
        ...
        // //保存odmo
        //  time_stamp_vec.emplace_back(point_cloud_first->measurement_time());        
