...
float Aligner::lidarOdomKNNError(const Pointcloud& base_pointcloud,
  ...
  float total_error = 0.0f;
  total_error =Aligner::kNNError( kdtree, base_pointcloud, k, max_dist, 0, base_pointcloud.size()-1);  
  return total_error;
}
...
void Aligner::lidarOdomTransform_(Lidar* lidar, Odom* odom) {
  ...
  if (!config_.local) {
    ...
    cout<<"Performing Global Optimization...                             "<<endl;
  ...
  cout<<"Performing Local Optimization...                             "<<endl;
