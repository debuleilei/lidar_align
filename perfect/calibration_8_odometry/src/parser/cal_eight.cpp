...
void cal_eight::compute_score(vector<Eigen::Affine3d>& trans,double& score)
{
    ...
    double tran_mean=(t_min/trans.size()).norm();
     ...
     cout<<"angle_mean:"<<50-angle_mean*100<<endl;
     cout<<"tran_mean:"<<50-tran_mean*1000<<endl;
...
void cal_eight::filter_ground(pcl::PointCloud<pcl::PointXYZ>::Ptr& points,pcl::PointCloud<pcl::PointXYZ>::Ptr& points_filter)
{
    ...
    // cout<<"filter ground"<<points_filter->points.size()<<endl;
...
void pcTransEstimator::convert(pcl::PointCloud<pcl::PointXYZRGB>& in, PointMatcher<float>::DataPoints& out)
{
	std::size_t n = in.size();
	out.features.resize(4,n);
...
void pcTransEstimator::estimate(pcl::PointCloud<pcl::PointXYZRGB>& ref, pcl::PointCloud<pcl::PointXYZRGB>& data, Eigen::Affine3d& t)
{
