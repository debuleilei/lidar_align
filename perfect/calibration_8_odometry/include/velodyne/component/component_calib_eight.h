...
class Comptransform_pointcloud_regist : public Component<adu::common::sensor::PointCloud,
adu::common::sensor::PointCloud> {
 public:
  int Init() override {
    std::cout<<"start Comptransform_pointcloud_regist!";
    ...
    if(!google::protobuf::TextFormat::ParseFromString(content, &velodyne_config)){
      LOG_ERROR<<"failed to parser lidar config config file!";
      return FAIL;
    }
    // std::cout<<"start 62";
    lidar_extrinsic_path_ = velodyne_config.lidars_filter_config_path();
    save_name_path_=velodyne_config.frame_id();
    // cout<<"path______"<<save_name_path_<<endl;
    // std::cout<<"start 64";
    ...
    for (int i = 0; i < size_; ++i) {
      ...
      cout<<"transform_pointcloud_regist**************init successfully"<<endl;
    return SUCC;
  }

  int Proc(
      const std::shared_ptr<adu::common::sensor::PointCloud const>& point_cloud_first,
      const std::shared_ptr<adu::common::sensor::PointCloud const>& point_cloud_second)override{
    ...
    point_cloud->Clear();
    if (_fusion->transform_lidar_regist(point_cloud_first,point_cloud_second, point_cloud,lidar_extrinsic_,save_name_path_)) {
      seq_++;
 ...
 private:
  ...
  std::string save_name_path_;
  Eigen::Affine3d lidar_extrinsic_; 
};

CLASS_LOADER_REGISTER_COMPONENT(Comptransform_pointcloud_regist);


class Comptransform_pointcloud : public Component<adu::common::sensor::PointCloud> {
 public:
  int Init() override {
    std::cout<<"start transform_pointcloud!";
     cybertron::proto::VelodyneConfig velodyne_config;
    // if (ParseConfig(&velodyne_config) != cybertron::SUCC) {
    //   LOG_ERROR << DRIVER_ERROR << LIDAR_CONF_PARSE_ERR << " Parse config file error";
    //   return cybertron::FAIL;
